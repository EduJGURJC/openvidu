...
public class FirefoxUser extends BrowserUser {
    ...
    public FirefoxUser(String userName, int timeOfWaitInSeconds) {
        this(userName, timeOfWaitInSeconds, null);
    }
    ...
    public FirefoxUser(String userName, int timeOfWaitInSeconds,
        ...
        if (additionalCapabilities != null && additionalCapabilities.size() > 0) {
            for (HashMap.Entry<String, Object> additionalCap : additionalCapabilities.entrySet()) {
                if ("firefoxVersion".equals(additionalCap.getKey())) {
                    capabilities.setCapability("version", additionalCap.getValue());
                } else {
                    capabilities.setCapability(additionalCap.getKey(), additionalCap.getValue());
                }
            }
        }
