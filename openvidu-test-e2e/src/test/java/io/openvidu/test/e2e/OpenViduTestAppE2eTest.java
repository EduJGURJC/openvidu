...
public class OpenViduTestAppE2eTest extends ElastestBaseTest {
    ...
    void setupBrowser(String browser) {
        ...
        switch (browser) {
        case "chrome":
            browserUser = new ChromeUser("TestUser", 50, false, additionalCapabilities);
            break;
        case "firefox":
            browserUser = new FirefoxUser("TestUser", 50, additionalCapabilities);
            break;
        case "opera":
        ...
        case "chromeAlternateScreenShare":
            ...
            break;
        case "chromeAsRoot":
            browserUser = new ChromeUser("TestUser", 50, true, additionalCapabilities);
            break;
        default:
            browserUser = new ChromeUser("TestUser", 50, false, additionalCapabilities);
        }
    ...
    void oneToOneAudioSession() throws Exception {
        ...
        user.getDriver().findElements(By.className("send-video-checkbox"))
                .forEach(el -> el.click());
        ...
        final int numberOfVideos = user.getDriver().findElements(By.tagName("video")).size();
        Assert.assertEquals("Expected 4 videos but found " + numberOfVideos, 4, numberOfVideos);
        Assert.assertTrue("Videos were expected to only have audio tracks",
                user.getEventManager().assertMediaTracks(
                        user.getDriver().findElements(By.tagName("video")), true, false));
    ...
    void oneToOneVideoSession() throws Exception {
        ...
        user.getDriver().findElements(By.className("send-audio-checkbox"))
                .forEach(el -> el.click());
        user.getDriver().findElements(By.className("join-btn"))
                .forEach(el -> el.sendKeys(Keys.ENTER));
    ...
    void oneRemoteSubscription() throws Exception {
        ...
        Assert.assertTrue("Video was expected to have audio and video tracks",
                user.getEventManager().assertMediaTracks(
                        user.getDriver().findElements(By.tagName("video")), true, true));
    ...
    void crossBrowserTest() throws Exception {
        ...
        Thread t = new Thread(() -> {
            ...
            try {
                ...
                final int numberOfVideos = user.getDriver().findElements(By.tagName("video"))
                        .size();
                Assert.assertEquals("Expected 2 videos but found " + numberOfVideos, 2,
                        numberOfVideos);
                Assert.assertTrue("Videos were expected to have audio and video tracks",
                        user.getEventManager().assertMediaTracks(
                                user.getDriver().findElements(By.tagName("video")), true, true));
    ...
    void subscribeUnsubscribeTest() throws Exception {
        ...
        user.getDriver().findElement(By.id("one2one-btn")).click();
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-0 .publish-checkbox"))
                .click();
        ...
        WebElement subscriberVideo = user.getDriver()
                .findElement(By.cssSelector("#openvidu-instance-0 video"));
        WebElement subBtn = user.getDriver()
                .findElement(By.cssSelector(("#openvidu-instance-0 .sub-btn")));
        subBtn.click();

        user.getWaiter().until(ExpectedConditions
                .not(ExpectedConditions.attributeToBeNotEmpty(subscriberVideo, "srcObject")));
        ...
        // Video unsubscribe
        subscriberVideo = user.getDriver()
                .findElement(By.cssSelector("#openvidu-instance-0 video"));
        Iterable<WebElement> firstVideo = Arrays.asList(subscriberVideo);
        user.getDriver().findElement(By.cssSelector(("#openvidu-instance-0 .sub-video-btn")))
                .click();
        Thread.sleep(1000);
        ...
        // Audio unsubscribe
        user.getDriver().findElement(By.cssSelector(("#openvidu-instance-0 .sub-audio-btn")))
                .click();
        Thread.sleep(1000);
    ...
    void publishUnpublishTest() throws Exception {
        ...
        for (WebElement video : user.getDriver().findElements(By.tagName("video"))) {
            user.getWaiter().until(ExpectedConditions
                    .not(ExpectedConditions.attributeToBeNotEmpty(video, "srcObject")));
    ...
    void changePublisherTest() throws Exception {
        ...
        user.getEventManager().on("streamPlaying", (event) -> {
            JsonObject stream = event.get("target").getAsJsonObject().get("stream")
                    .getAsJsonObject();
        ...
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-0 .change-publisher-btn"))
                .click();
    ...
    void moderatorCapabilitiesTest() throws Exception {
        ...
        // Add subscriber
        user.getDriver().findElement(By.id("add-user-btn")).click();
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-1 .publish-checkbox"))
                .click();

        // Add and configure moderator (only subscribe)
        user.getDriver().findElement(By.id("add-user-btn")).click();
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-2 .publish-checkbox"))
                .click();
        ...
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-2 .force-unpub-btn"))
                .click();
        ...
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-2 .force-disconnect-btn"))
                .click();
    ...
    void streamPropertyChangedEventTest() throws Exception {
        ...
        user.getEventManager().on("streamPropertyChanged", (event) -> {
            ...
            System.out.println(
                    "Publisher dimensions: " + event.get("newValue").getAsJsonObject().toString());
            System.out.println("Real dimensions of viewport: " + expectedDimensions);
            threadAssertions
                    .add("videoDimensions".equals(event.get("changedProperty").getAsString()));
            threadAssertions.add(
    ...
    void localRecordTest() throws Exception {
        ...
        WebElement recordBtn = user.getDriver()
                .findElement(By.cssSelector("#openvidu-instance-0 .publisher-rec-btn"));
        recordBtn.click();
        ...
        user.getWaiter().until(
                ExpectedConditions.elementToBeClickable(By.cssSelector("#recorder-preview video")));

        try {
            user.getWaiter().until(waitForVideoDuration(
                    user.getDriver().findElement(By.cssSelector("#recorder-preview video")), 4));
        } catch (Exception e) {
            System.out.println(getBase64Screenshot(user));
            Assert.fail();
        }
        ...
        user.getWaiter().until(ExpectedConditions
                .numberOfElementsToBe(By.cssSelector("#recorder-preview video"), 0));
    ...
    void remoteComposedRecordTest() throws Exception {
        ...
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Error [404]"));
        ...
        Thread.sleep(500);
        WebElement resolutionField = user.getDriver()
                .findElement(By.id("recording-resolution-field"));
        ...
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording started [" + sessionName + "]"));
        ...
        user.getDriver().findElement(By.id("get-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording got [" + sessionName + "]"));
        ...
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording list [" + sessionName + "]"));

        // Stop ongoing recording
        user.getDriver().findElement(By.id("stop-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording stopped [" + sessionName + "]"));
    ...
    void remoteIndividualRecordTest() throws Exception {
        ...
        Assert.assertFalse("Recording folder " + recPath + " shouldn't exist",
                new File(recPath).exists());
    ...
    void remoteRecordAudioOnlyVideoOnlyTest() throws Exception {
        ...
        Thread t = new Thread(() -> {
            ...
            user2.getDriver().findElement(By.id("add-user-btn")).click();
            user2.getDriver()
                    .findElement(By.cssSelector("#openvidu-instance-1 .send-audio-checkbox"))
                    .click();

            // Join Firefox users
            user2.getDriver().findElements(By.className("join-btn"))
                    .forEach(el -> el.sendKeys(Keys.ENTER));
            ...
            try {
                ...
                Assert.assertEquals("Expected 8 videos in Firefox user but found " + nVideos, 8,
                        nVideos);
        ...
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-0 .send-audio-checkbox"))
                .click();

        // Chrome user audio-only
        user.getDriver().findElement(By.id("add-user-btn")).click();
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-1 .send-video-checkbox"))
                .click();
        ...
        user.getDriver().findElement(By.id("start-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording started [" + SESSION_NAME + "]"));
        user.getEventManager().waitUntilEventReaches("recordingStarted", 2);
        ...
        user.getDriver().findElement(By.id("stop-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording stopped [" + SESSION_NAME + "]"));
        user.getEventManager().waitUntilEventReaches("recordingStopped", 2);
        ...
        user.getDriver().findElement(By.id("start-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording started [" + SESSION_NAME + "-1]"));
        ...
        user.getDriver().findElement(By.id("stop-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording stopped [" + SESSION_NAME + "-1]"));
        ...
        user.getDriver().findElement(By.id("start-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording started [" + SESSION_NAME + "-2]"));
        ...
        user.getDriver().findElement(By.id("stop-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording stopped [" + SESSION_NAME + "-2]"));
        ...
        user.getDriver().findElement(By.id("start-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording started [" + SESSION_NAME + "-3]"));
        ...
        user.getDriver().findElement(By.id("stop-recording-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeToBe(By.id("api-response-text-area"),
                "value", "Recording stopped [" + SESSION_NAME + "-3]"));
        ...
        Recording recording = new OpenVidu(OPENVIDU_URL, OPENVIDU_SECRET)
                .getRecording(SESSION_NAME);
    ...
    void restApiFetchForce() throws Exception {
        ...
        log.info(
                "REST API: Fetch all, fetch one, force disconnect, force unpublish, close session");
        ...
        user.getWaiter().until(ExpectedConditions.attributeContains(By.id("api-response-text-area"),
                "value", "Number: 0. Changes: false"));
        ...
        Thread.sleep(1000);
        System.out.println(getBase64Screenshot(user));
        System.out.println(user.getDriver().findElement(By.id("api-response-text-area"))
                .getAttribute("value"));
        ...
        String response = user.getDriver().findElement(By.id("api-response-text-area"))
                .getAttribute("value");
        JsonObject json = JsonParser.parseString(response.split("%")[1]).getAsJsonArray().get(0)
                .getAsJsonObject();
        String connectionId = json.keySet().iterator().next();
        String streamId = json.get(connectionId).getAsJsonArray().get(0).getAsJsonObject()
                .get("streamId").getAsString();

        // Fetch all sessions (no change)
        user.getDriver().findElement(By.id("list-sessions-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeContains(By.id("api-response-text-area"),
                "value", "Number: 1. Changes: false"));
    ...
    void videoFilterTest() throws Exception {
        ...
        // Configure publisher
        user.getDriver().findElement(By.id("add-user-btn")).click();
        user.getDriver().findElements(By.cssSelector("#openvidu-instance-0 .subscribe-checkbox"))
                .get(0).click();
        user.getDriver().findElements(By.cssSelector("#openvidu-instance-0 .send-audio-checkbox"))
                .get(0).click();
        ...
        int numberOfVideos = user.getDriver().findElements(By.tagName("video")).size();
        Assert.assertEquals("Expected 2 videos but found " + numberOfVideos, 2, numberOfVideos);
        Assert.assertTrue("Videos were expected to have a video only track",
                user.getEventManager().assertMediaTracks(
                        user.getDriver().findElements(By.tagName("video")), false, true));

        WebElement subscriberVideo = user.getDriver()
                .findElement(By.cssSelector("#openvidu-instance-1 video"));
        ...
        user.getWaiter()
                .until(ExpectedConditions.attributeContains(By.id("filter-response-text-area"),
                        "value", "Error [You don't have permissions to apply a filter]"));
        ...
        user.getWaiter().until(ExpectedConditions.attributeContains(
                By.id("filter-response-text-area"), "value", "has no filter applied in session"));
        ...
        user.getDriver().findElement(By.id("apply-filter-btn")).click();
        user.getWaiter().until(ExpectedConditions
                .attributeContains(By.id("filter-response-text-area"), "value", "Filter applied"));

        // Try to apply another filter
        user.getDriver().findElement(By.id("apply-filter-btn")).click();
        user.getWaiter()
                .until(ExpectedConditions.attributeContains(By.id("filter-response-text-area"),
                        "value", "already has a filter applied in session"));
        ...
        user.getDriver().findElement(By.id("exec-filter-btn")).click();
        user.getWaiter().until(ExpectedConditions.attributeContains(
                By.id("filter-response-text-area"), "value", "Filter method executed"));
        ...
        user.getWaiter().until(ExpectedConditions
                .attributeContains(By.id("filter-response-text-area"), "value", "Filter removed"));
        user.getEventManager().waitUntilEventReaches("streamPropertyChanged", 6);
        Thread.sleep(1000);
    ...
    void videoFilterEventsTest() throws Exception {
        ...
        int numberOfVideos = user.getDriver().findElements(By.tagName("video")).size();
        Assert.assertEquals("Expected 2 videos but found " + numberOfVideos, 2, numberOfVideos);
        Assert.assertTrue("Videos were expected to have only a video track",
                user.getEventManager().assertMediaTracks(
                        user.getDriver().findElements(By.tagName("video")), false, true));
        ...
        user.getWaiter().until(ExpectedConditions.attributeContains(
                By.id("filter-response-text-area"), "value", "Filter event listener added"));
        ...
        user.getWaiter().until(ExpectedConditions.attributeContains(
                By.id("filter-response-text-area"), "value", "Filter event listener removed"));
    ...
    void openViduJavaClientTest() throws Exception {
        ...
        Assert.assertFalse(
                "OV.fetch() should return false if OV.createSession() has not been called",
                OV.fetch());
        List<Session> sessions = OV.getActiveSessions();
        Assert.assertEquals("Expected no active sessions but found " + sessions.size(), 0,
                sessions.size());

        SessionProperties properties = new SessionProperties.Builder()
                .customSessionId(customSessionId).mediaMode(MediaMode.ROUTED)
        ...
        Assert.assertFalse("Session.fetch() should return false after OpenVidu.createSession()",
                session.fetch());
        Assert.assertFalse("OpenVidu.fetch() should return false after OpenVidu.createSession()",
                OV.fetch());
        sessions = OV.getActiveSessions();
        Assert.assertEquals("Expected 1 active session but found " + sessions.size(), 1,
                sessions.size());
        ...
        TokenOptions tokenOptionsSubscriber = new TokenOptions.Builder()
        ...
        Assert.assertFalse("Session.fetch() should return false until a user has connected",
                session.fetch());

        // Set client data 1
        WebElement clientDataInput = user.getDriver()
                .findElement(By.cssSelector("#client-data-input-0"));
        ...
        user.getDriver().findElement(By.id("session-settings-btn-0")).click();
        Thread.sleep(1000);
        WebElement tokeInput = user.getDriver()
                .findElement(By.cssSelector("#custom-token-div input"));
        tokeInput.clear();
        ...
        user.getDriver().findElement(By.cssSelector("#openvidu-instance-0 .active-audio-checkbox"))
                .click();
        ...
        Assert.assertEquals("Wrong recording mode", RecordingMode.ALWAYS,
                session.getProperties().recordingMode());
        ...
        Assert.assertEquals(
                ...
                2, session.getActiveConnections().size());
        ...
        // Verify publishers
        Assert.assertEquals(
                "Expected 1 publisher for connection " + connectionModerator.getConnectionId()
                        + " but found " + connectionModerator.getPublishers().size(),
                1, connectionModerator.getPublishers().size());
        Assert.assertEquals(
                "Expected 0 publishers for connection " + connectionSubscriber.getConnectionId()
                        + " but found " + connectionSubscriber.getPublishers().size(),
                0, connectionSubscriber.getPublishers().size());

        // Verify subscribers
        Assert.assertEquals(
                "Expected 0 subscribers for connection " + connectionModerator.getConnectionId()
                        + " but found " + connectionModerator.getSubscribers().size(),
                0, connectionModerator.getSubscribers().size());
        Assert.assertEquals(
                "Expected 1 subscriber for connection " + connectionSubscriber.getConnectionId()
                        + " but found " + connectionSubscriber.getSubscribers().size(),
                1, connectionSubscriber.getSubscribers().size());
        Assert.assertEquals("Publisher and subscriber should have same streamId",
                connectionModerator.getPublishers().get(0).getStreamId(),
                connectionSubscriber.getSubscribers().get(0));

        // Verify server and client data
        Assert.assertEquals("Server data doesn't match", serverDataModerator,
                connectionModerator.getServerData());
        Assert.assertEquals("Server data doesn't match", serverDataSubscriber,
                connectionSubscriber.getServerData());
        Assert.assertEquals("Client data doesn't match", clientDataModerator,
                connectionModerator.getClientData());
        ...
        Assert.assertTrue("Session.fetch() should return true after publisher changed",
                session.fetch());
        Assert.assertFalse("OpenVidu.fetch() should return false after Session.fetch()",
                OV.fetch());
        ...
        Assert.assertEquals("{\"width\":" + obj.get("width").getAsLong() + ",\"height\":"
                + (obj.get("height").getAsLong()) + "}", pub.getVideoDimensions());
        Assert.assertEquals(new Integer(30), pub.getFrameRate());
        ...
        try {
            ...
            Assert.assertEquals("Wrong HTTP status on OpenVidu.startRecording()", 404,
                    e.getStatus());
        }
        ...
        try {
            ...
            Assert.assertEquals("Wrong HTTP status on OpenVidu.startRecording()", 406,
                    e.getStatus());
        } finally {
        ...
        try {
            recordingProperties = new RecordingProperties.Builder().hasAudio(false).hasVideo(false)
                    .build();
            OV.startRecording(session.getSessionId(), recordingProperties);
        } catch (OpenViduHttpException e) {
            Assert.assertEquals("Wrong HTTP status on OpenVidu.startRecording()", 422,
                    e.getStatus());
        }
        try {
        ...
        try {
            ...
            Assert.assertEquals("Wrong HTTP status on OpenVidu.startRecording()", 409,
                    e.getStatus());
        }
        ...
        Assert.assertEquals("Recording id and name should be equal", recording.getId(),
                recording.getName());
        Assert.assertEquals("Recording id and sessionId should be equal", session.getSessionId(),
                recording.getId());

        // Check ongoing recording properties
        Assert.assertEquals("Wrong recording session id", session.getSessionId(),
        ...
        Assert.assertEquals("Wrong recording status", Recording.Status.started,
                recording.getStatus());
        ...
        this.checkIndividualRecording("/opt/openvidu/recordings/" + customSessionId + "/",
                recording, 2, "opus", "vp8", false);
        ...
        recordingProperties = new RecordingProperties.Builder()
                .outputMode(Recording.OutputMode.COMPOSED).recordingLayout(RecordingLayout.BEST_FIT)
                .resolution("1280x720").hasVideo(true).hasAudio(false).name(customRecordingName)
                .build();
        ...
        Assert.assertEquals("Wrong recording id", session.getSessionId() + "-1",
                recording2.getId());
        Assert.assertEquals("Wrong recording session id", session.getSessionId(),
                recording2.getSessionId());
        ...
        Assert.assertEquals("Wrong recording layout", RecordingLayout.BEST_FIT,
                recording2.getRecordingLayout());
    ...
    void restApiTest() throws Exception {
        ...
        CustomHttpClient restClient = new CustomHttpClient(OPENVIDU_URL, "OPENVIDUAPP",
                OPENVIDU_SECRET);
        ...
        restClient.rest(HttpMethod.POST, "/api/sessions", body, HttpStatus.SC_OK, true,
                "{'id': 'STR', 'createdAt': 0}");
        // Default values
        JsonObject res = restClient.rest(HttpMethod.POST, "/api/sessions", "{}", HttpStatus.SC_OK,
                true, "{'id': 'STR', 'createdAt': 0}");
        restClient.rest(HttpMethod.DELETE, "/api/sessions/" + res.get("id").getAsString(),
        ...
        restClient.rest(HttpMethod.GET, "/api/sessions/CUSTOM_SESSION_ID", null, HttpStatus.SC_OK,
                true,
        ...
        Assert.assertEquals(
                "JSON return value from /api/tokens should have equal srtings in 'id' and 'token'",
                res.get("id").getAsString(), token1);
        Assert.assertEquals("Wrong session parameter", "CUSTOM_SESSION_ID",
                res.get("session").getAsString());
        ...
        restClient.rest(HttpMethod.POST, "/api/signal", body, HttpStatus.SC_BAD_REQUEST);
        body = "{'session':'CUSTOM_SESSION_ID'}";
        restClient.rest(HttpMethod.POST, "/api/signal", body, HttpStatus.SC_NOT_ACCEPTABLE); // No
                                                                                             // connections
        ...
        restClient.rest(HttpMethod.POST, "/api/recordings/start", body,
        ...
        res = restClient.rest(HttpMethod.POST, "/api/sessions", "{'mediaMode':'RELAYED'}",
        ...
        // 405
        restClient.rest(HttpMethod.POST, "/api/recordings/stop", body,
                HttpStatus.SC_METHOD_NOT_ALLOWED);

        // 404
        restClient.rest(HttpMethod.POST, "/api/recordings/stop/NOT_EXISTS", body,
                HttpStatus.SC_NOT_FOUND);

        // 200
        restClient.rest(HttpMethod.DELETE, "/api/recordings/CUSTOM_SESSION_ID",
                HttpStatus.SC_CONFLICT);
        restClient.rest(HttpMethod.POST, "/api/recordings/stop/CUSTOM_SESSION_ID", body,
        ...
        restClient.rest(HttpMethod.GET, "/api/recordings/CUSTOM_SESSION_ID", null, HttpStatus.SC_OK,
                true,
                "{'id':'STR','sessionId':'STR','name':'STR','outputMode':'STR','recordingLayout':'STR','hasAudio':false,'hasVideo':false,'resolution':'STR','createdAt':0,'size':0,'duration':0,'url':'STR','status':'STR'}");
        restClient.rest(HttpMethod.GET, "/api/recordings", null, HttpStatus.SC_OK, true,
        ...
        restClient.rest(HttpMethod.DELETE, "/api/sessions/NOT_EXISTS/stream/NOT_EXISTS",
                HttpStatus.SC_BAD_REQUEST);
        restClient.rest(HttpMethod.DELETE, "/api/sessions/CUSTOM_SESSION_ID/stream/NOT_EXISTS",
                HttpStatus.SC_NOT_FOUND);
        res = restClient.rest(HttpMethod.GET, "/api/sessions/CUSTOM_SESSION_ID", null,
                HttpStatus.SC_OK, true,
        ...
        String streamId = res.get("connections").getAsJsonObject().get("content").getAsJsonArray()
                .get(0).getAsJsonObject().get("publishers").getAsJsonArray().get(0)
                .getAsJsonObject().get("streamId").getAsString();
        restClient.rest(HttpMethod.DELETE, "/api/sessions/CUSTOM_SESSION_ID/stream/" + streamId,
                HttpStatus.SC_NO_CONTENT);

        final String connectionId = res.get("connections").getAsJsonObject().get("content")
                .getAsJsonArray().get(0).getAsJsonObject().get("connectionId").getAsString();
        ...
        body = "{'session':'CUSTOM_SESSION_ID','to':['" + connectionId
                + "'],'type':'server2','data':'SERVER EVENT!'}";
        restClient.rest(HttpMethod.POST, "/api/signal", body, HttpStatus.SC_OK);
        user.getEventManager().waitUntilEventReaches("signal:server2", 1);
        Assert.assertEquals("", 1,
                user.getDriver()
                        .findElements(
                                By.xpath("//*[text()='server - signal:server2 - SERVER EVENT!']"))
                        .size());

        /** DELETE /api/sessions/<SESSION_ID>/connection/<CONNECTION_ID> **/
        restClient.rest(HttpMethod.DELETE, "/api/sessions/NOT_EXISTS/connection/NOT_EXISTS",
                HttpStatus.SC_BAD_REQUEST);
        restClient.rest(HttpMethod.DELETE, "/api/sessions/CUSTOM_SESSION_ID/connection/NOT_EXISTS",
                HttpStatus.SC_NOT_FOUND);
        restClient.rest(HttpMethod.DELETE,
                "/api/sessions/CUSTOM_SESSION_ID/connection/" + connectionId,
                HttpStatus.SC_NO_CONTENT);
    ...
    void kurentoReconnectTest() throws Exception {
        ...
        Assert.assertTrue(
                "Alert message wrong. Expected to contain: \"" + alertMessage
                        + "\". Actual message: \"" + alert.getText() + "\"",
        ...
        OV.fetch();
        sessions = OV.getActiveSessions();
        Assert.assertEquals("Expected 1 active sessions but found " + sessions.size(), 1,
                sessions.size());
        ...
        Assert.assertEquals(
                "Expected 2 active connections but found " + session.getActiveConnections(), 2,
                session.getActiveConnections().size());
        int pubs = session.getActiveConnections().stream()
                .mapToInt(con -> con.getPublishers().size()).sum();
        int subs = session.getActiveConnections().stream()
                .mapToInt(con -> con.getSubscribers().size()).sum();
        ...
        Assert.assertEquals(
                "Expected 2 active connections but found " + session.getActiveConnections(), 2,
                session.getActiveConnections().size());
        pubs = session.getActiveConnections().stream().mapToInt(con -> con.getPublishers().size())
                .sum();
        subs = session.getActiveConnections().stream().mapToInt(con -> con.getSubscribers().size())
                .sum();
        ...
        double differenceInDuration = Math
                .abs(rec.getDuration() - ((recEndTime - recStartTime) / 1000));
        Assert.assertTrue(
                "Recording duration exceeds valid value. Expected no more than 0.2 seconds, got "
                        + differenceInDuration,
                differenceInDuration < 0.2);

        this.checkIndividualRecording("/opt/openvidu/recordings/TestSession/", rec, 1, "opus",
                "vp8", true);

        WebElement pubBtn = user.getDriver()
                .findElements(By.cssSelector("#openvidu-instance-1 .pub-btn")).get(0);
        pubBtn.click();
    ...
    void webhookTest() throws Exception {
        ...
        try {
            ...
            WebElement allowedFilterInput = user.getDriver()
                    .findElement(By.id("allowed-filter-input"));
            allowedFilterInput.clear();
            allowedFilterInput.sendKeys("ZBarFilter");
            user.getDriver().findElement(By.id("add-allowed-filter-btn")).click();
            ...
            Assert.assertEquals("Wrong number of properties in event 'sessionCreated'", 2 + 1,
                    event.keySet().size());
            ...
            Assert.assertEquals("Wrong number of properties in event 'webrtcConnectionCreated'",
                    10 + 1, event.keySet().size());

            event = CustomWebhook.waitForEvent("recordingStatusChanged", 10);
            Assert.assertEquals("Wrong number of properties in event 'recordingStatusChanged'",
                    11 + 1, event.keySet().size());
            ...
            Assert.assertEquals("Wrong recording outputMode in webhook event", 0,
                    event.get("size").getAsLong());
            Assert.assertEquals("Wrong recording outputMode in webhook event", 0,
                    event.get("duration").getAsLong());
            ...
            Assert.assertNull("Wrong recording reason in webhook event (should be null)",
                    event.get("reason"));

            // Filter event webhook
            user.getDriver().findElement(By.cssSelector("#openvidu-instance-0 .filter-btn"))
                    .click();
            Thread.sleep(500);
            WebElement input = user.getDriver().findElement(By.id("filter-type-field"));
            input.clear();
            ...
            Assert.assertEquals("Wrong number of properties in event 'recordingStatusChanged'",
                    12 + 1, event.keySet().size());
            Assert.assertEquals("Wrong recording status in webhook event", "ready",
                    event.get("status").getAsString());
            Assert.assertTrue("Wrong recording outputMode in webhook event",
                    event.get("size").getAsLong() > 0);
            Assert.assertTrue("Wrong recording outputMode in webhook event",
                    event.get("duration").getAsLong() > 0);
            Assert.assertEquals("Wrong recording reason in webhook event", "sessionClosedByServer",
                    event.get("reason").getAsString());
            Assert.assertEquals("Wrong recording reason in webhook event", rec.getCreatedAt(),
                    event.get("startTime").getAsLong());
            ...
            Assert.assertEquals("Wrong number of properties in event 'sessionDestroyed'", 5 + 1,
                    event.keySet().size());
            Assert.assertEquals("Wrong session destroyed reason in webhook event",
                    "sessionClosedByServer", event.get("reason").getAsString());
    ...
    void ipCameraTest() throws Exception {
        ...
        try {
            ...
            // Wrong session [404]
            restClient.rest(HttpMethod.POST, "/api/sessions/WRONG_SESSION/connection", "{}",
                    HttpStatus.SC_NOT_FOUND);

            // Init a session and publish IP camera AS FIRST PARTICIPANT
            restClient.rest(HttpMethod.POST, "/api/sessions",
                    "{'customSessionId':'IP_CAM_SESSION'}", HttpStatus.SC_OK, true,
                    "{'id': 'STR', 'createdAt': 0}");
            ...
            JsonObject response = restClient.rest(HttpMethod.POST,
                    "/api/sessions/IP_CAM_SESSION/connection", ipCamBody, HttpStatus.SC_OK, true,
            ...
            Assert.assertEquals("Wrong serverData property", "MY_IP_CAMERA",
                    response.get("serverData").getAsString());
            Assert.assertEquals("Wrong platform property", "IPCAM",
                    response.get("platform").getAsString());
            Assert.assertEquals("Wrong role property", "PUBLISHER",
            ...
            JsonObject ipCamPublisher = response.get("publishers").getAsJsonArray().get(0)
                    .getAsJsonObject();
            Assert.assertEquals("Wrong number of properties in IPCAM publisher", 4,
                    ipCamPublisher.size());
            ...
            Assert.assertEquals("Wrong number of properties in MediaOptions", 10,
                    mediaOptions.size());
            Assert.assertTrue("Wrong adaptativeBitrate property",
                    mediaOptions.get("adaptativeBitrate").getAsBoolean());
            ...
            restClient.rest(HttpMethod.DELETE,
                    "/api/sessions/IP_CAM_SESSION/stream/"
                            + ipCamPublisher.get("streamId").getAsString(),
                    HttpStatus.SC_METHOD_NOT_ALLOWED);

            // Can delete the connection [204]
            restClient.rest(HttpMethod.DELETE, "/api/sessions/IP_CAM_SESSION/connection/"
                    + response.get("connectionId").getAsString(), HttpStatus.SC_NO_CONTENT);

            response = CustomWebhook.waitForEvent("webrtcConnectionDestroyed", 1);
            Assert.assertEquals("Wrong reason in webrtcConnectionDestroyed event",
                    "forceDisconnectByServer", response.get("reason").getAsString());
            ...
            restClient.rest(HttpMethod.POST, "/api/recordings/start",
                    "{'session':'TestSession','name':'audioVideo','hasAudio':true,'hasVideo':true}",
                    HttpStatus.SC_OK);
            ...
            Thread.sleep(4000);
            restClient.rest(HttpMethod.POST, "/api/recordings/stop/TestSession-1",
                    HttpStatus.SC_OK);
            user.getEventManager().waitUntilEventReaches("recordingStopped", 1);
            CustomWebhook.waitForEvent("recordingStatusChanged", 1); // Stopped
            CustomWebhook.waitForEvent("recordingStatusChanged", 1); // Ready
            ...
            String recPath = restClient.rest(HttpMethod.GET, "/config", HttpStatus.SC_OK)
                    .get("openviduRecordingPath").getAsString();
            ...
            restClient.rest(HttpMethod.POST, "/api/sessions/TestSession/connection", ipCamBody,
                    HttpStatus.SC_OK, true,
                    "{'connectionId':'STR','createdAt':0,'location':'STR','platform':'STR','role':'STR','serverData':'STR','clientData':'STR','publishers':[],'subscribers':[]}");
            ...
            user.getDriver()
                    .findElement(By.cssSelector("#openvidu-instance-0 .force-disconnect-btn"))
                    .click();
            ...
            response = restClient.rest(HttpMethod.POST, "/api/sessions/TestSession/connection",
            ...
            restClient.rest(HttpMethod.DELETE,
                    "/api/sessions/TestSession/connection/" + connectionId,
                    HttpStatus.SC_NO_CONTENT);
            ...
            restClient.rest(HttpMethod.POST, "/api/sessions", "{'customSessionId':'TestSession'}",
                    HttpStatus.SC_OK);
...
//			this.checkIndividualRecording(recPath + recId + "/", recording, 1, "opus", "vp8", true);
            ...
            restClient.rest(HttpMethod.DELETE, "/api/sessions/TestSession",
                    HttpStatus.SC_NO_CONTENT);
    ...
    private ExpectedCondition<Boolean> waitForVideoDuration(WebElement element,
        ...
        return new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver input) {
                return element.getAttribute("duration").matches(durationInSeconds - 1
                        + "\\.[5-9][0-9]{0,5}|" + durationInSeconds + "\\.[0-5][0-9]{0,5}");
            }
        };
    }
    ...
    private boolean checkVideoAverageRgbGray(Map<String, Long> rgb) {
        ...
        return (rgb.get("r") < 50) && (rgb.get("g") < 50) && (rgb.get("b") < 50)
                && (Math.abs(rgb.get("r") - rgb.get("g")) <= 2)
                && (Math.abs(rgb.get("r") - rgb.get("b")) <= 2)
    ...
    private void gracefullyLeaveParticipants(int numberOfParticipants) throws Exception {
        ...
        for (int j = 1; j <= numberOfParticipants; j++) {
            ...
            user.getEventManager().waitUntilEventReaches("connectionDestroyed",
                    accumulatedConnectionDestroyed);
        }
    }
    ...
    private boolean recordedFileFine(File file, Recording recording) throws IOException {
        this.checkMultimediaFile(file, recording.hasAudio(), recording.hasVideo(),
                recording.getDuration(), recording.getResolution(), "aac", "h264", true);
        ...
        try {
            ...
            Assert.assertEquals("Resolution (" + recording.getResolution()
                    + ") of recording entity is not equal to real video resolution ("
                    + realResolution + ")", recording.getResolution(), realResolution);
    ...
    private void checkIndividualRecording(String recPath, Recording recording,
            int numberOfVideoFiles, String audioDecoder, String videoDecoder, boolean checkAudio)
            throws IOException {

        // Should be only 2 files: zip and metadata
        File folder = new File(recPath);
        Assert.assertEquals(
                "There are more than 2 files (ZIP and metadata) inside individual recording folder "
                        + recPath + ": " + Arrays.toString(folder.listFiles()),
                2, folder.listFiles().length);
        ...
        List<File> unzippedWebmFiles = new Unzipper().unzipFile(recPath,
                recording.getName() + ".zip");

        Assert.assertEquals(
                "Expecting " + numberOfVideoFiles + " videos inside ZIP file but "
                        + unzippedWebmFiles.size() + " found: " + unzippedWebmFiles.toString(),
                numberOfVideoFiles, unzippedWebmFiles.size());
        ...
        try {
            ...
            log.error("Cannot read JSON sync metadata file from {}. Error: {}",
                    jsonSyncFile.getAbsolutePath(), e.getMessage());
            Assert.fail(
                    "Cannot read JSON sync metadata file from " + jsonSyncFile.getAbsolutePath());
            return;
        }
        ...
        for (File webmFile : unzippedWebmFiles) {
            ...
            Assert.assertTrue("Couldn't find in JSON sync object information for webm file "
                    + webmFile.getName(), found);

            log.info("Duration of {} according to sync metadata json file: {} s",
                    webmFile.getName(), durationInSeconds);
            this.checkMultimediaFile(webmFile, recording.hasAudio(), recording.hasVideo(),
                    durationInSeconds, recording.getResolution(), audioDecoder, videoDecoder,
                    checkAudio);
            webmFile.delete();
        }

        Assert.assertEquals(
                "Size of recording entity (" + recording.getSessionId()
                        + ") is not equal to real file size (" + totalFileSize + ")",
                recording.getSize(), totalFileSize);

        jsonSyncFile.delete();
    }

    private void checkMultimediaFile(File file, boolean hasAudio, boolean hasVideo, double duration,
            String resolution, String audioDecoder, String videoDecoder, boolean checkAudio)
            throws IOException {
        ...
        if (hasVideo) {
            if (checkAudio) {
                if (hasAudio) {
                    ...
                    Assert.assertTrue(
                            metadata.getAudioDecoder().toLowerCase().contains(audioDecoder));
                } else {
            ...
            if (resolution != null) {
                Assert.assertEquals(resolution,
                        metadata.getVideoWidth() + "x" + metadata.getVideoHeight());
            }
        ...
        Assert.assertTrue(
                "Difference between recording entity duration (" + duration
                        + ") and real video duration (" + metadata.getDuration()
                        + ") is greater than " + difference + "  in file " + file.getName(),
